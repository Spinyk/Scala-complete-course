  ...
  case class StuffBox(books: List[Book] = Nil,
   ...
   // Поправьте логику метода
    ...
    private def putStuffInRightBox(item: Stuff, stuffBox: StuffBox): StuffBox = item match {
      case watches: Watches if watches.cost > 1000 => stuffBox.copy(watches = watches :: stuffBox.watches)
      case boots: Boots if boots.brand.equals("Converse") || boots.brand.equals("Adidas") => stuffBox.copy(boots = boots :: stuffBox.boots)
      case book:Book if book.isInteresting => stuffBox.copy(books = book :: stuffBox.books)
      case junk@_ => stuffBox.copy(junk = junk :: stuffBox.junk)
    }

    def findMyKnife(stuffBox: StuffBox): Boolean = stuffBox match {
      case stuffBox if stuffBox.junk.contains(Knife) => true
      case _ => false
    }
