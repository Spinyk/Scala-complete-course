...
object CouriersWithComprehension extends App {
  ...
  def serveAddresses(addresses: List[Address], couriers: List[Courier]) = couriers.flatMap(courier => {
    val trafficDegree = traffic().degree;
    (0 until courier.canServe).withFilter(t => trafficDegree < 5 && t < addresses.length).map(t => addresses(t))
  })


  def traffic(): Traffic = new Traffic(Math.random() * 10)

  def printServedAddresses(addresses: List[Address], couriers: List[Courier]) = serveAddresses(addresses, couriers).foreach(a => println(a.postIndex))
